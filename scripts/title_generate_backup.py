...
def _apply_modifiers(
    ...
    ``--slot noun:plural`` turns ``{noun}`` into ``{noun:plural}``.
    Placeholders that already have modifiers are left untouched.
    """
    if not modifiers:
        return templates
...
def main() -> None:
    ...
    args.flavor_arg, args.final_count = parse_flavor_arg(
        parser, args, available, title.categorized_title_flavors(pre_args.story)
    )
    ...
    for slot, groups in args.parsed_slot_sources.items():
        for grp in groups:
            for src in grp:
    ...
    # {noun} into {noun:plural} for this run.
    ...
    if args.enumerate_all:
        ...
        for tmpl, _lvl in tmpl_list:
            for result_str in title.expand_all(tmpl, grammar):
